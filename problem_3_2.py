...
svm_ova = SVC(C=12, gamma="scale", decision_function_shape='ovr')
svmstart = time.time()
svm_ova.fit(x_train, y_train)
svm_ova_pred = svm_ova.predict(x_test)
svmstsop = time.time()
svm_accuracy = accuracy_score(y_test, svm_ova_pred)
svm_c_time = svmstsop - svmstart

print('Accuracy for OVA SVM: ' + str(svm_accuracy * 100))
print('Total execution time for OVA SVM: ' + str(svm_c_time))

# # One vs All multiclass aka OVR
#
...
# ova_c_time = ovastsop - ovastart
#
# print('Accuracy for OVA: ' + str(ova_accuracy * 100))
# print('Total execution time for OVA: ' + str(ova_c_time))
